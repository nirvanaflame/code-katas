...
class AlphabetSymmetryTest {
  ...
  void shouldReturnArrayOfSymmetriesLength() {
    assertArrayEquals(new int[] {4, 3, 1}, countSymmetry(new String[] {"abode", "ABc", "xyzD"}));

    assertArrayEquals(new int[] {4, 3, 0}, countSymmetry(new String[] {"abide", "ABc", "xyz"}));
  ...
  void testCleverSolution() {
    assertArrayEquals(
        new int[] {1, 26, 1, 3},
        cleverCountSymmetry(
            new String[] {"encode", "abcdefghijklmnopqrstuvwxyzd", "xyzD", "ABmD"}));
  }
