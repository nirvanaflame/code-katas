 ...
 *
 * <p>We need a function to collect these numbers, that may receive two integers a, b that defines
...
class SumPoweredDigitsTest {
  ...
  void shouldReturnEmptyList() {
    assertEquals(List.of(), sumDigPow(90, 100));
  }

  @Test
  void shouldReturnSimpleNumbersAnd89() {
    assertEquals(List.of(1L, 2L, 3L, 4L, 5L, 6L, 7L, 8L, 9L, 89L), sumDigPow(1, 101));
  }

  @Test
  void testBestSolutionFromCodewars() {
    assertEquals(List.of(1L, 2L, 3L, 4L, 5L, 6L, 7L, 8L, 9L, 89L), sumDigPowBest(1, 121));
  }
  ...
  private List<Long> sumDigPow(long a, long b) {
    ...
    for (long i = a; i <= b; i++) {
      if (i < 10) {
        ...
        int pawned =
            IntStream.rangeClosed(1, num.length())
                .map(j -> (int) Math.pow(Character.getNumericValue(num.charAt(j - 1)), j))
                .sum();
  ...
  private List<Long> sumDigPowBest(long a, long b) {
    return LongStream.rangeClosed(a, b).filter(this::isValid).boxed().collect(Collectors.toList());
  }

  private boolean isValid(long n) {
    String s = String.valueOf(n);
    return IntStream.range(0, s.length())
            .mapToDouble(i -> Math.pow(Character.getNumericValue(s.charAt(i)), i + 1))
            .sum()
        == n;
  }
}
