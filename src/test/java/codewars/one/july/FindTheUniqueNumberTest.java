 ...
 *
 * <p>Kata.findUniq(new double[]{ 1, 1, 1, 2, 1, 1 }); // => 2 Kata.findUniq(new double[]{ 0, 0,
 * 0.55, 0, 0 }); // => 0.55 It’s guaranteed that array contains at least 3 numbers.
 */
class FindTheUniqueNumberTest {
  ...
  private double findUnique(double[] array) {
    ...
    return countValues.entrySet().stream()
